...
Uniform3fv:                proc(loc: i32, count: u32, value: ^f32) #cc_c;
Uniform4fv:                proc(loc: i32, count: u32, value: ^f32) #cc_c;
...
GenFramebuffers:          proc(n: i32, ids: ^u32) #cc_c;
BindFramebuffer:          proc(target: u32, framebuffer: u32) #cc_c;
FramebufferTexture2D:     proc(target, attachment, textarget: u32, texture: u32, level: i32) #cc_c;
FramebufferRenderbuffer:  proc(target, attachment, renderbuffertarget: u32, renderbuffer: u32) #cc_c;
BlitFramebuffer:          proc(srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1: i32, mask: u32, filter: u32) #cc_c;

Viewport:                 proc(x, y: i32, width, height: i32) #cc_c;
...
get_uniform_location :: proc(program: u32, name: string) -> i32 {
	return GetUniformLocation(program, ^byte(&name[0]));
}
